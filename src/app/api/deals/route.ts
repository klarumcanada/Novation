...
export async function GET() {
  ...
  const { data, error } = await supabase
    .from('deals')
    .select('id, status, seller_confirmed_next, buyer_confirmed_next, created_at, updated_at, thread_id, seller_id, buyer_id, mga_id')
    .or(`seller_id.eq.${user.id},buyer_id.eq.${user.id}`)
    .order('updated_at', { ascending: false })

  if (error) return NextResponse.json({ error: error.message }, { status: 400 })

  // Hydrate advisor names
...
export async function POST(request: NextRequest) {
  ...
  // Get current user's advisor record to determine intent and mga_id
  ...
  const { data: deal, error } = await supabase
    .from('deals')
    .insert({ seller_id, buyer_id, mga_id, thread_id: thread_id ?? null })
    .select()
    .single()
